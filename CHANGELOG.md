...
- **Publishing runs from CI on a `v*` tag.** npm records a provenance attestation tying the tarball to the workflow run and the commit it came from, which is the trust marker shown on the package page, and the version cannot drift from the tag — the workflow refuses to publish when `package.json` disagrees with the tag it was triggered by. `access` and `provenance` stay in `publishConfig` so a local publish behaves the same way.
- **CI runs `check-dist`**, so the `'use client'` banner and the script-tag build's `isSupported` are guarded on every push rather than only in a local `pnpm validate`.
- **Fixed the visual job.** It ran `pnpm visual` without building, which was fine until the examples specs arrived: they load `dist/stipple.global.js` by relative path, so the job would have gone red on the next push. Confirmed by moving `dist` aside and watching it fail.

### Tooling

- **Two more browser tests and one more baseline.** A portrait screenshot, because the field genuinely lays out differently on a short wide canvas than a tall narrow one. Reduced motion asserted on values rather than pixels: a still field looks exactly like a settled one, and what matters is that it drew once and is not running.
- **Worker parity compared on distribution, not pixels.** The worker owns its global scope, so the seeded generator and frozen clock never reach it and it refuses `tick` — it drives its own loop. Adding a seeding hook to the protocol to buy pixel equality would be production API existing only for a test, so the two are compared on where the field ends up: main thread 8.12% ink, worker 7.99%, occupancy grids 0.012 apart.
...
- **A colour ramp described the sphere, not the shape.** Ramp positions were computed once from the dispersed layout and never revisited, so after a morph the gradient still said where each particle came from. Measured as the correlation between the colour a particle is painted and its actual distance from centre: 0.92 while dispersed, 0.04 once morphed — statistically independent of position, which is what made it read as noise. The shape-side ramp is now measured from the target positions and blended in by `morph`, so the gradient arrives with the shape; a swap crossfades between the two shapes' ramps on its own clock. Only `radius` changes. `index` never depended on position, and a shape is flat — `shapeZ` is a couple of pixels of jitter against a spread radius in the hundreds — so measuring `depth` against it would collapse the ramp to one flat colour rather than fix anything.
